        ...
        BlocListener<AuthBloc, AuthState>(
          listener: (context, state) {
            if (state.status == AuthStatus.unauthenticated) {
              ...
              // AutoRouter.of(context).pushAndPopUntil(const SigninScreenRoute(),
              //     predicate: (route) => false);
              // Navigator.of(context).pushNamedAndRemoveUntil(
              //     SigninScreen.routeName, (route) => false);
            } else if (state.status == AuthStatus.authenticated) {
              AutoRouter.of(context).push(const EnterScreenRoute());
              // AutoRouter.of(context).pushAndPopUntil(const EnterScreenRoute(),
              //     predicate: (route) => false);
              // Navigator.of(context).pushNamedAndRemoveUntil(
        ...
        BlocListener<PasswordCubit, PasswordState>(
          ...
          listener: (context, state) {
            if (state.status == PasswordStatus.success) {
              ...
              AutoRouter.of(context).pushAndPopUntil(const StoreScreenRoute(),
                  predicate: (route) => false);
              // Navigator.of(context).pushNamedAndRemoveUntil(
